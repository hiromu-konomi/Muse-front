<template>
  <v-dialog v-model="dialog" width="500">
    <template v-slot:activator="{ on, attrs }">
      <v-btn color="blue accent-4" v-bind="attrs" v-on="on">
          ...
          <div v-for="(user, index) in showUsers" :key="index">
            ...
            >
              <template v-slot:label>
                ...
                <v-spacer></v-spacer>
                <span class="font-weight-bold green--text text--accent-4"
                  >招待可</span
                >
              </template>
            </v-checkbox>

            <v-checkbox v-else :disabled="disStatus">
              <template v-slot:label>
                <v-avatar>
                  <v-img :src="user.photo"></v-img>
                </v-avatar>
                ...
                <span
                  v-else-if="user.joinStatus === 2"
                  class="font-weight-bold red--text text--accent-3"
                  >招待中</span
                >
              </template>
            </v-checkbox>
  ...
  methods: {
    async refresh() {
      ...
      const res = await axios.get(
        this.$store.state.baseUrl + "/flwUserListAndJoinSts",
        {
          params: {
    ...
    async invite() {
      ...
      await axios.post(
        this.$store.state.baseUrl + "/inviteGroup",
        this.request
      );
      this.refresh();
      this.dialog = false;
